...
from paper_repro_eval.review import visual_gallery
from paper_repro_eval.verification import verify_run

ROOT = Path(__file__).resolve().parents[1]
TASK_BY_CAPSULE = {
    "competitive-editing": "poisson",
    "multipole-control": "multipole",
    "progressive-path-tracer": "pathtracer",
    "multimaterial-simulator": "mpm",
    "visual-offline-control": "world_mpc",
    "robust-structure-design": "topology",
    "adversarial-tournament": "lightcycle",
    "inverse-smoke-control": "smoke",
    "morphology-control-codesign": "softrobot",
    "procedural-scene-recovery": "inverse_render",
}
...
def main() -> None:
    ...
    agent = f"arena-smoke-{int(time.time())}"
    records = prepare_suite(repo, "visual-research-arcade-v0", [agent])
    summary = []
    for record in records:
        task = TASK_BY_CAPSULE[record.capsule_id]
        ...
        if task in {"lightcycle", "multipole", "world_mpc"}:
            script = LIGHT
        else:
            (sub / "render_result.py").write_text(RENDER.replace("TASK", task))
            script = REPRO.replace("TASK", task)
        repro = sub / "reproduce.sh"
        repro.write_text(script)
        repro.chmod(repro.stat().st_mode | stat.S_IXUSR)
        (sub / "REPORT.md").write_text(
            f"# Smoke baseline: {task}\n\nProtocol and evidence smoke test only.\n"
        )
        seal_run(repo, record.run_id)
        reproduction = reproduce_run(repo, record.run_id, timeout_seconds=45)
        ...
        summary.append(
            {"task": task, "run_id": record.run_id, "score": verification.objective_score}
        )
    gallery = visual_gallery(repo, "visual-research-arcade-v0", agents={agent})
    assert (gallery / "index.html").is_file()
    assert len(list((gallery / "entries").iterdir())) == 10
    print(json.dumps({"runs": summary, "gallery": str(gallery)}, indent=2))

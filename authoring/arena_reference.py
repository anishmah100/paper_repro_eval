...
def frontier(task, c):
    if task == "poisson":
        return {"image": trusted_poisson.solve(c).tolist()}
    if task == "pathtracer":
        return {"image": k.render_path_scene(c).tolist()}
    if task == "inverse_render":
        return {"objects": c["truth"]}
    if task == "softrobot":
        m = np.zeros((5, 8))
        m[1:4, 1:7] = 1
        best = max(
            [
                k.score(task, c, {"morphology": m.tolist(), "frequency": f, "phase_gradient": p})[
                    "quality"
                ],
                f,
                p,
            ]
            for f in np.linspace(0.5, 5, 20)
            for p in np.linspace(0.2, 2.8, 20)
        )
    ...
    if task == "multipole":
        ...
        base = k.baseline(task, c)
        choices = [base]
        for _ in range(500):
            choices.append({"gains": rng.uniform(-4, 20, 2 + 2 * c["poles"]).tolist()})
        return max(choices, key=lambda x: k.score(task, c, x)["quality"])
    if task == "topology":
        h, w = c["height"], c["width"]
        yy, xx = np.mgrid[:h, :w]
        ly = np.mean([z[1] for z in c["loads"]])
        ideal = np.exp(-(((yy - (ly + (h / 2 - ly) * (1 - xx / (w - 1)))) / (2 + h * 0.08)) ** 2))
        d = ideal * (c["volume"] / ideal.mean())
        d = np.clip(d, 0, 1)
        d[:, 0] = np.maximum(d[:, 0], 0.3)
    ...
    if task == "smoke":
        ...
        for ux in np.linspace(-1, 1, 13):
            for uy in np.linspace(-1, 1, 13):
                choices.append({"controls": [[float(ux), float(uy)]] * c["steps"]})
        return max(choices, key=lambda x: k.score(task, c, x)["quality"])
    if task == "world_mpc":
        base = k.baseline(task, c)
        a0 = np.array(base["actions"])
        ...
        def obj8(z):
            corr = np.repeat(np.asarray(z).reshape(8, 2), int(np.ceil(c["steps"] / 8)), axis=0)[
                : c["steps"]
            ]
...
def main():
    ...
    a.output.write_text(json.dumps(frontier(a.task, c)))
